-- ============================================================================
-- V8：补回历史种子数据 + 清理 V5 留下的旧表
--
-- 这版本把以前散落在 src/main/resources/sql/init.sql 里的数据搬进 Flyway，
-- 让"git pull + docker compose up"一次就能拿到完整的可演示库。
--
--   1. DROP 掉 V5 重命名留下的 chat_message_v1（空库部署上没用，老库迁移
--      也已过 1-2 周观察期）
--   2. 19 篇历史日记（2026-05-26 ~ 2026-06-14，user_id=1）
--   3. 3 条朋友圈 + 1 条点赞 + 2 条评论
--   4. 显式把两个用户的 is_first_login 锁回 1（防御性）
--
-- 注意：种子数据，仅适合当前固定两用户的私有部署。
--       moment 行显式指定了 id=1/2/3，因为后面的 moment_like/moment_comment
--       要按 id 引用——空库下自增也是 1/2/3，写死只是更稳。
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1) 清理 V5 留下的旧聊天表
-- ---------------------------------------------------------------------------
DROP TABLE IF EXISTS `chat_message_v1`;

-- ---------------------------------------------------------------------------
-- 2) 历史日记 (2026-05-26 ~ 2026-06-14)
-- ---------------------------------------------------------------------------
-- 2026-05-26
...
-- ---------------------------------------------------------------------------
-- 3) 朋友圈（id 显式写死，因为下面的 like / comment 要按 id 引用）
-- ---------------------------------------------------------------------------
INSERT INTO `moment` (`id`, `user_id`, `content`, `create_time`) VALUES
(1, 1, '今天姐姐捧着我的脸和我说话，心跳加速到爆表 😊',                                       '2026-06-07 21:30:00'),
(2, 1, '有些人就像星星，远远看着就很美，靠近了反倒怕被灼伤',                                  '2026-06-09 23:00:00'),
(3, 1, '第一个自己写的项目马上就要上线了，用户只有我和他两个人，这就够了',                     '2026-06-13 10:00:00');

-- 点赞：潘佩雪给王水群第 1 条 moment 点赞
INSERT INTO `moment_like` (`moment_id`, `user_id`, `create_time`)
VALUES (1, 2, '2026-06-13 16:53:17');

-- 评论：潘佩雪在 moment 1 和 moment 3 下的评论
...
-- ---------------------------------------------------------------------------
-- 4) 显式锁定两个种子用户的 is_first_login = 1
--   （V1 INSERT 时已经走的是默认值 1，这里只是防御性兜底，避免被先前调试改过）
-- ---------------------------------------------------------------------------
UPDATE `user` SET `is_first_login` = 1 WHERE `id` IN (1, 2);
